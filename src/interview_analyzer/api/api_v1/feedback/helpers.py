...
from interview_analyzer.api.api_v1.feedback.model import QuestionAndAnswerWithoutFeedback, Feedback, STARFeedback
from interview_analyzer.api.api_v1.enrich.model import EnrichedTranscript, LabelledUtterance
from interview_analyzer.core.open_ai.prompts.star_method import star_method_system_prompt
...
def collect_answer(i: int, question_number: int, utterances: List[LabelledUtterance]) -> Tuple[int, str]:
    ...
    while i < len(utterances):
        ...
        if cur.speaker_type.lower() == "interviewee" and cur.question_number == question_number and \
                (cur.speech_type.lower() == "answer" or cur.speech_type.lower() == "continuation"):
