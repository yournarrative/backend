...
def extract_audio(input_file: str, output_filetype: str) -> str:
    ...
    try:
        (ffmpeg.input(input_file).output(output_file, format=output_filetype, b="1M", y="-y").run())
    except Exception as e:
        logger.error(f"Failed to extract {output_filetype} file from .webm file: {e}, stderr: {e.stderr}")
        raise e
    else:
        logger.debug(
            f"Successfully extracted .m4a file from .webm file, elapsed time: {str(time.time() - start_time)} seconds"
        )
        return output_file
