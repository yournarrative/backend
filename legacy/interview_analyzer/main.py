...
from legacy.interview_analyzer.api.api_v1.convert.endpoint import convert_audio_filetype
from legacy.interview_analyzer.api.api_v1.email.endpoint import send_email
from legacy.interview_analyzer.api.api_v1.enrich.endpoint import enrich_transcript
from legacy.interview_analyzer.api.api_v1.enrich.model import EnrichedTranscript
from legacy.interview_analyzer.api.api_v1.feedback.endpoint import question_answer_feedback
...
from legacy.interview_analyzer.api.api_v1.transcribe.endpoint import transcribe_bytes_file_speech_to_text
from legacy.interview_analyzer.api.api_v1.transcribe.model import SimpleTranscript
from legacy.interview_analyzer.app_lifespan_management import cleanup_app_state, init_app_state
from legacy.interview_analyzer.connectors.rds import crud
from legacy.interview_analyzer.connectors.s3.crud import read_file_from_s3, upload_file_to_s3
from legacy.interview_analyzer.connectors.s3.schema import ProcessS3Request
from legacy.interview_analyzer.utils.files import cleanup_temp_file
...
async def _process_s3_file_into_rds(data: ProcessS3Request, request: Request):
    try:
        ...
        interview_uuid = await crud.get_interview_id_from_rds_using_s3_path(
            async_session=request.app.state.database_access_layer.async_session, s3_bucket=data.bucket, s3_key=data.key
        )
        ...
        if data.send_email_when_finished:
            email_address: str = await crud.get_email_from_rds_using_interview_uuid(
                async_session=request.app.state.database_access_layer.async_session, interview_uuid=interview_uuid
            )
            await send_email(interview=interview, email_to=email_address, state=request.app.state)
        return Response(status_code=200)
    except Exception as e:
        logger.error(e)
        return HTTPException(status_code=500)
